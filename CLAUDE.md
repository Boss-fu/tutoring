...
- **分頁整併（重要）**：教師端已無獨立「兼職薪資」「收入分析」分頁。導覽只剩 首頁／課次與月曆／學生課務／檔案中心／學費與收入／學習狀況／家長端預覽／個人課表／新增·管理行程。
  ...
  - **新增課次類型（`#formKind`）＝四大分類下拉（2026-09 起）**：**私人家教(priv)／公司家教(comp)／兼職(parttime)／正職上班(fulltime)**（已移除「家教課」「個人行程」選項）。下拉值＝**分類(category)**，與**儲存 kind**（`data-kind`：tutoring/parttime/fulltime/personal）分離：`CAT_TO_KIND`＝priv/comp→tutoring、parttime→parttime、fulltime→fulltime。表單欄位仍以 `#lessonForm[data-kind]` ＋ `.kf/.kt/.kp/.kfu/.kpe` 顯示（priv/comp 同為 tutoring shape，共用 `.kt`/`.ktblock`）。`openEdit(item,arg)`：編輯既有→分類用 `itemCat(item)`、kind 用 `itemKind(item)`（依日期欄位判 lessons/work_shifts/calendar_events，**編輯時不改 `data-kind` 避免跨表**）；新建→`arg` 為分類，`data-kind=CAT_TO_KIND`。submit/delete/repeat 都以 `curKind()`（`data-kind`）決定資料表（`kindTable()`），標題/確認字以 `CAT_LABEL[分類]`。存檔分流：priv/comp→`lessons`（存 `category`）、parttime→`work_shifts`、fulltime→`calendar_events`(`kind='fulltime'`)；編輯既有 personal 事件改分類則存 `category`、`kind` 維持 personal（僅時段標記、不計金額）。**`#formKind.onchange` 新建才切 `data-kind`＋補預設；編輯只重算預設色，不可呼叫會 `form.reset()` 的 openEdit。**
    - **匯入／行事曆行程一鍵轉正式課次（`#toCourse`／`#convertCevent`）**：編輯既有 `calendar_events`（personal/fulltime）且分類為 priv/comp 時，`.editor-actions` 顯示「➜ 轉為正式課次」。按下＝把表單切為 `data-kind='tutoring'`（露出學生／時數／家教明細）、以標題比對自動選學生、清 `#lessonId`（改走 insert）、記 `#convertCevent`＝原事件 id。submit 建立 `lessons` 後，若 `convertCevent` 有值即 `calendar_events.delete().eq('id',conv)` 移除原行事曆行程（未按儲存前不刪，非破壞性）。公司家教學生（如奕捷／昀筠）需先存在於 `students` 才能轉。
    - **每筆行程可自訂顏色（`#formColorOn`＋`#formColor`）**：勾選才覆蓋分類色，存 `color`（三表皆有 `color` 欄）；`eventStyle(l)`／`personalTypeStyle(e)` 有 `color` 就用、文字色以 `textOn(hex)` 依亮度決定黑/白。未勾＝用分類預設色。
    - **所有類型皆可標出席狀態（`#formStatus`：實到/請假/遲到/缺席）**：`lessons` 原有、`work_shifts`／`calendar_events` 新增 `status` 欄。`statusOff(s)`＝請假/缺席在月曆與明細淡化（`.is-off`），遲到/實到不淡化；家教遲到仍計費（`billable`）。三表皆有 `status`（work_shifts/calendar_events 僅視覺標記，不影響金額）。**需先套用 `supabase/20260916_event_color_status.sql`（加 `color`/`status`/`category` 欄）。**`openEdit(item,kind)` 依類型帶欄位；刪除／submit 都以 `#formKind` 值決定資料表。**注意 `openEdit` 尾端有一層 `openEditWithSafeDefaults` 包裝（強制新家教課 `formStatus='attended'`），該包裝必須 `function(l,kind){...(l,kind)...}` 把 `kind` 一起轉傳，否則所有 `openEdit(item,'personal'|'fulltime'|'parttime')` 都會被吃掉 kind 而彈回家教。** 月曆 `renderCalendar` 畫家教課次(私人紅／公司紫，依 `lessonCat`)、兼職(黃 `#fbe7a2`)、正職(藍 `.ce-full`)；個人行程(misc)一律不畫。點 `data-cevent` 可編輯。**同一天內四種事件都會合併後依 `start_time` 由早到晚排序**（`cell.sort`；無時間者排最後）；`showDayDetail` 當天明細清單同樣依 `start_time` 排序。**月曆格子上的事件只顯示名稱、不顯示時間前綴（避免小格子被 HH:MM 佔滿變雜）；排序仍依 `start_time`。點日期看的「當天明細」`.di-time` 仍保留時間。****月曆改為「四大分類＝四種色」（2026-09 起）**：中央 `CATEGORY_STYLE`＝**私人家教 `priv` 紅 `#fbd6d6`／公司家教 `comp` 紫 `#e6d7fb`／兼職 `part` 黃 `#fbe7a2`／正職上班 `full` 藍 `#d3e6fb`／個人行程 `misc` 灰 `#eceff3`（灰＝月曆隱藏）**。分類規則：`COMPANY_NAMES=/奕捷|昀筠|筠昀/`、`PRIVATE_NAMES=/桓安|禹安/`；`catText(text,kind)`＝私人名→priv、公司名→comp、含諾貝爾→part、`kind==='fulltime'` 或含「茲茲/天母/士林/B班/實驗/輔導/正課/教務」→full、其餘→misc。`lessonCat(l)`＝家教課次(`lessons`)以學生姓名判 comp/priv（家教就是家教，公司安排的也走家教系統，只是配紫色）。`eventStyle(l)` 回傳 `CATEGORY_STYLE[lessonCat(l)]` 色；`personalTypeStyle(e)` 對 `calendar_events` 回 `catText` 分類與四色＋`[3]=key`。**`renderCalendar`/`showDayDetail`/`renderCalendarLegend` 都會 `if(c[3]==='misc')return` 跳過個人行程（不畫、不列、不進圖例）**。圖例順序 `['priv','comp','part','full']`、只列本月出現者。`.ce-full`/`.ce-personal` class 保留（供既有測試/相容），顏色一律以 inline style 覆蓋。（此為教師端月曆專用，家長端 parent-preview 仍用 `COURSE_PALETTE` 逐課配色，兩邊獨立、不需同步。）主模組 load 另抓 `calEvents`、`employers`。**家教課次狀態 `STATUS_LABEL`＝實到/請假/缺席/遲到（`late`）**：遲到視同有上課，`billable`＝`status==='attended'||'late'`，仍計費、計時、計入學費單與收入；請假/缺席不計。**需先套用 `supabase/20260907_calendar_events.sql` 建 `calendar_events` 表**（否則正職/個人存檔會失敗；load 端已防呆空跑）。
    - **點日期看當天明細（手機主操作）＝ `showDayDetail` / `#calendarDayDetail` / `.day-item`**：手機月曆格子太窄塞不下中文課名（曾因兩段 `@media(max-width:680px)` 打架、後一段把 `.event` 設回 `font-size:9.5px` 導致課名被切成「周…」超難讀；已修正為兩段都 `font-size:0`、`height:10px` 乾淨色塊）。所以手機月曆只顯示**無字色塊**（顏色對照看下方圖例），改為「點日期→在月曆下方展開當天全部課次／行程清單」才是可讀入口。每筆 `.day-item`（`.di-time`＋`.di-body`）可點開對應編輯器：家教 `data-open-lesson`→`openEdit(l)`、兼職 `data-open-shift`→`openEdit(s,'parttime')`、正職/個人 `data-open-cevent`→`openEdit(ev,ev.kind)`；底部「＋ 在這天新增」`data-add-day` 會 `openEdit()` 後帶入該日期。點 `.event` 色塊本身仍直接開編輯（`if(e.target.closest('.event'))return`）。
...
- 月曆拖曳／複製：teacher.html 尾端模組 `mMove`/`mCopy` 處理家教課次、`mMoveShift`/`mCopyShift` 處理兼職班次、**`mMoveCevent`/`mCopyCevent` 處理正職/個人/匯入行程（`calendar_events`，改 `event_date`）**；三種月曆事件皆為 `draggable`（`.event[data-id]`/`[data-shift]`/`[data-cevent]`），拖曳 payload 帶 `kind`（lesson/shift/cevent）與 `id`。拖放到其他日期後由 `mChooseAction` 依類型顯示對話框，明確選擇「移動／複製／取消」，不會直接改動資料；主模組重載入口 `window.BOSSFU_RELOAD`。
- **每週重複（`#repeatWeekly` / `repeatWeekly()`）**：課次編輯器 `.editor-actions` 內的「📅 每週重複」按鈕。依目前表單 `#formKind` 與各欄位，`prompt` 問週數（預設 12、1–52），把這筆行程以「同星期同時段、每 7 天」建立到未來 N 週（家教→`lessons`、兼職→`work_shifts`、正職/個人→`calendar_events`）。編輯既有行程時第 1 週保留原筆（`startI=1`），新建時含當週（`startI=0`）。routine 課程一鍵鋪滿。
...
## Supabase 資料表（線上實際）
`profiles`(role: teacher/parent)、`students`、`parent_students`(多對多)、`lessons`(另有 `status`實到/請假/缺席/遲到、`color`自訂色、`category`priv/comp)、`messages`(有 `author_role`、`parent_id`)、`student_files`(有 `uploader_id`、`parent_id`)、`issued_invoices`(student_id, month, PK)、`site_settings`(key/value，如 `finance_profile`、`salary_income`＝正職各月實領 JSON)、`employers`(兼職單位；`default_rate`/`labor_insurance`/`health_insurance`)、`work_shifts`(兼職班次；`employer_id`/`work_date`/`hours`/`rate`；另有 `status`/`color`)、`push_subscriptions`(user_id/endpoint/p256dh/auth，推播訂閱)、`teacher_prefs`(key/value jsonb，老師專用設定；`key='personal_schedule'`＝個人班表雲端備份；RLS 只有 `is_teacher()` 可讀寫，家長讀不到，故**不放 site_settings**；SQL＝`supabase/20260909_teacher_prefs.sql`)、`calendar_events`(老師專用一站式行事曆：`kind`=fulltime正職上班/personal個人行程、`title`/`event_date`/`start_time`/`end_time`/`note`；另有 `status`/`color`/`category`(priv/comp/parttime/fulltime)；只放時段標記、不計金額；SQL＝`supabase/20260907_calendar_events.sql`＋`supabase/20260916_event_color_status.sql`，RLS 用 `is_teacher()`)。Storage bucket：`exam-papers`（上傳已移除前端 10MB 限制；Storage 端仍有預設上限）。
